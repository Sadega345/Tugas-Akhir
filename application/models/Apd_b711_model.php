...
// Listing
 ...
 $data=$this->db->query('SELECT p.jurusan,SUM(pd.ts_2)AS Tot_Ts2,SUM(pd.ts_1)AS Tot_Ts1,SUM(pd.ts)AS Tot_Ts,(SELECT SUM(jml_dana)
...
(SELECT SUM(jml_dana)FROM dana_penelitian WHERE tahun=2015)AS Jum_dana2015,(SELECT SUM(jml_dana)FROM dana_penelitian WHERE tahun=2016)AS Jum_dana2016 
FROM penelitian_dosen pd INNER JOIN dana_penelitian d INNER JOIN prodi_tbl p ON pd.kd_jns=d.jns_dana WHERE p.kode_fakultas="F001"
');
 return $data->result_array();
 }
