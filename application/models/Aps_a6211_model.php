...
// Listing
 ...
 $jml1=$this->db->query('SELECT sum(ts_2) as jum2,sum(ts_1) as jum1,sum(ts) as jum,sum(ts_2+ts_1+ts) as totals1 FROM perolehan_dana WHERE id_dana=1 and kd_prodi="p002"');
 ...
 $jml2=$this->db->query('select sum(ts_2) as jml2,sum(ts_1) as jml1,sum(ts) as jml,sum(ts_2+ts_1+ts) as totals2 from perolehan_dana where id_dana=2 and kd_prodi="p002"');
 ...
 $jml4=$this->db->query('select sum(ts_2) as jumlah2,sum(ts_1) as jumlah1,sum(ts) as jumlah,sum(ts_2+ts_1+ts) as totals4 from perolehan_dana where id_dana=4 and kd_prodi="p002"');
 return $jml4->result_array();
 }

 public function total() {
 $total=$this->db->query('select sum(ts_2) as total1,sum(ts_1) as total2,sum(ts) as total3,sum(ts_2+ts_1+ts) as totals from perolehan_dana where kd_prodi="p002"');
