...
// Listing
 ...
 $dana1=$this->db->query('SELECT id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total1 FROM perolehan_dana WHERE id=1 and kd_prodi='."'$kdprodi'");
 return $dana1->result_array();
 }

 public function listing2() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana2=$this->db->query('SELECT id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total2 FROM perolehan_dana WHERE id=2 and kd_prodi='."'$kdprodi'");
 return $dana2->result_array();
 }

 public function listing3() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana3=$this->db->query('SELECT id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total3 FROM perolehan_dana WHERE id=3 and kd_prodi='."'$kdprodi'");
 ...
 public function listing4() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana4=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total4 from perolehan_dana where id=4 and kd_prodi='."'$kdprodi'");
 return $dana4->result_array();
 }

 public function listing5() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana5=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total5 from perolehan_dana where id=5 and kd_prodi='."'$kdprodi'");
 return $dana5->result_array();
 }

 public function listing6() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana6=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total6 from perolehan_dana where id=6 and kd_prodi='."'$kdprodi'");
 ...
 public function listing7() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana7=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total7 from perolehan_dana where id=7 and kd_prodi='."'$kdprodi'");
 return $dana7->result_array();
 }

 public function listing8() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana8=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total8 from perolehan_dana where id=8 and kd_prodi='."'$kdprodi'");
 return $dana8->result_array();
 }

 public function listing9() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $dana9=$this->db->query('select id,ts_2,ts_1,ts,sum(ts_2+ts_1+ts) as total9 from perolehan_dana where id=9 and kd_prodi='."'$kdprodi'");
