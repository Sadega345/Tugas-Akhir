...
// Listing
 ...
 $data=$this->db->query('SELECT TS_2,TS_1,TS,SUM(TS_2+TS_1+TS) AS JML,KD_JNS FROM kegiatan_pkm WHERE KD_JNS=3 AND kd_prodi="p002"');
 return $data->result_array();
 }

 public function luarkemristekdikti() {
 $data=$this->db->query('SELECT TS_2,TS_1,TS,SUM(TS_2+TS_1+TS) AS JML,KD_JNS FROM kegiatan_pkm WHERE KD_JNS=4 AND kd_prodi="p002"');
 return $data->result_array();
 }
