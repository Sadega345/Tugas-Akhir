...
// Listing
 ...
 public function depdiknas() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $data=$this->db->query('SELECT TS_2,TS_1,TS,SUM(TS_2+TS_1+TS) AS JML,KD_JNS FROM kegiatan_pkm WHERE KD_JNS=6 AND kd_prodi='."'$kdprodi'");
 return $data->result_array();
 }

 public function dalamdepdiknas() {
 	$kdprodi = $this->session->userdata('kd_prodi');
 $data=$this->db->query('SELECT TS_2,TS_1,TS,SUM(TS_2+TS_1+TS) AS JML,KD_JNS FROM kegiatan_pkm WHERE KD_JNS=7 AND kd_prodi='."'$kdprodi'");
 return $data->result_array();
 }
